...
from diffusion_policy.gym_util.multistep_wrapper import LIBEROMultiStepWrapper
...
from diffusion_policy.libero_extension.envs import OffScreenRenderEnv
            ...
            for steps in range(self.max_steps // self.n_action_steps):
                ...
                obs_dict = dict_apply(np_obs_dict, 
                    lambda x: torch.from_numpy(x).to(
                        device=device))
                obs_dict['agentview_image'] = torch.flip(obs_dict['agentview_image'], dims=[-2])
                obs_dict['robot0_eye_in_hand_image'] = torch.flip(obs_dict['robot0_eye_in_hand_image'], dims=[-2])
                # run policy
