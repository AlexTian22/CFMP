...
from diffusion_policy.gym_util.video_recording_wrapper import MetaWorldVideoRecordingWrapper, VideoRecorder
    ...
    def __init__(self,
        ...
        def env_fn(task_name):
            return MultiStepWrapper(
                MetaWorldVideoRecordingWrapper(
    ...
    def run(self, policy: BaseImagePolicy, save_video=True):
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Metaworld {self.task_name}  Env", leave=False, mininterval=self.tqdm_interval_sec):
            ...
            while not done:
                ...
                if 'variance' in np_action_dict:
                    variance.append(np_action_dict['variance'])
                    nfe.append(np_action_dict['nfe'])
        ...
        if 'variance' in np_action_dict:
            log_data['variance'] = np.mean(variance)
            log_data['nfe'] = np.mean(nfe)
